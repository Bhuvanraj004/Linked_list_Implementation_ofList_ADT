#include<iostream.h> #include<conio.h> #include<stdlib.h> struct node
{
int element; struct node *next;
}*list=NULL,*p; struct node *find(int);
struct node *findprevious(int); void insert(int X);
void deletion(int X); void display();
void main()
{
int data,ch; clrscr();
cout<<"1.Insert\n2.Deletion\n3.Display\n4.Exit"; do
...
{
case 1:
cout<<"enter the element to insert"; cin>>data;
insert(data);
 

break; case 2:
cout<<"enter the element to delete"; cin>>data;
deletion(data); break;
case 3:
...
{
struct node *temp;
temp=(struct node*)malloc(sizeof(struct node)); p=findprevious(X);
if(p->next!=NULL)
{
temp=p->next;
p->next=temp->next;
cout<<"\n the deleted element is"<<temp->element; free(temp);
}
else
...
void display()
{
if(list->next==NULL) cout<<"list is empty"; else
{
p=list->next;
